...
function DailyChart({ data, actualKey, targetKey, valueFormatter = num }) {
  const max = Math.max(1, ...data.flatMap((item) => [item[actualKey] || 0, item[targetKey] || 0]));
  return <div className="aim-chart" role="img" aria-label="Daily target and actual chart">
    <div className="aim-chart-legend"><span><i className="target" />Target</span><span><i className="actual" />Actual</span></div>
    <div className="aim-chart-bars">{data.map((item) => <div className="aim-chart-day" key={item.date} title={`${item.date}: Target ${valueFormatter(item[targetKey])}, Actual ${valueFormatter(item[actualKey])}`}><div className="aim-chart-columns"><span className="target" style={{ height: `${(item[targetKey] || 0) / max * 100}%` }} /><span className="actual" style={{ height: `${(item[actualKey] || 0) / max * 100}%` }} /></div><small>{new Date(`${item.date}T00:00:00`).toLocaleDateString("en-IN", { day: "numeric", month: "short" })}</small></div>)}</div>
  </div>;
}
...
function AimMasterPage() {
  ...
  return <div className="app-page">
    ...
    <div className="aim-chart-grid"><Card><div className="section-card-head"><span className="eyebrow">DAILY PERFORMANCE</span><h2>Targets vs Actuals</h2><p>Daily activity across leads, communication and C1–C4 stages.</p></div><DailyChart data={data?.daily || []} actualKey="actual" targetKey="target" /></Card><Card><div className="section-card-head"><span className="eyebrow">DAILY REVENUE</span><h2>Target vs Won</h2><p>Daily revenue goals compared with closed-won value.</p></div><DailyChart data={data?.daily || []} actualKey="revenueActual" targetKey="revenueTarget" valueFormatter={money} /></Card></div>
    ...
    <div className="aim-stat-grid aim-bottom-metrics"><StatCard label="Active Deals" value={num(pipeline.activeDeals)} helper="Open or on hold" /><StatCard label="Average Probability" value={`${num(pipeline.averageProbability)}%`} helper="Across all deals" /><StatCard label="Build Handoffs" value={num(pipeline.buildHandoffs)} helper="Won work handed over" /><StatCard label="Upcoming Follow-ups" value={num(data?.upcoming?.length)} helper="Next scheduled actions" /></div>
  </div>;
}
