...
function AimMasterPage() {
  ...
  return <div className="app-page">
    ...
    <div className="aim-readiness-strip"><Badge tone="success">A + TARGETS LIVE</Badge><span>Lead actuals and A follow-ups are live. C1/C2 and C3/C4 actuals connect in Phases 5–6 using the same permanent lead IDs.</span></div>
    ...
    <div className="aim-dashboard-grid"><Card><div className="section-card-head"><span className="eyebrow">TARGET ACHIEVEMENT</span><h2>CTA & Stage Progress</h2><p>A lead capture is now live. Communication and commercial actuals become live in I and M.</p></div><div className="aim-progress-list">{[["Leads", "leads"], ["Emails", "emails"], ["WhatsApp", "messages"], ["Calls", "calls"], ["Meetings", "meetings"], ["C1", "c1"], ["C2", "c2"], ["C3", "c3"], ["C4", "c4"], ["Proposals", "proposals"]].map(([label, key]) => <Progress key={key} label={label} actual={actual[key] || 0} target={target[key] || 0} />)}</div></Card><Card><div className="section-card-head"><span className="eyebrow">LEAD MIX</span><h2>Top Industries</h2><p>Live sector distribution from A Master Leads.</p></div><div className="commercial-metrics">{(data?.sectorMix || []).length ? data.sectorMix.map((item) => <div key={item.industry}><span>{item.industry}</span><strong>{num(item.count)}</strong></div>) : <div><span>No leads captured yet</span><strong>0</strong></div>}</div><div className="section-card-head" style={{ marginTop: 20 }}><span className="eyebrow">COMMERCIAL</span><h2>Pipeline & Revenue</h2><p>C3/C4 values become live in Phase 6.</p></div><div className="commercial-metrics"><div><span>Open Pipeline</span><strong>{money(pipeline.openValue)}</strong></div><div><span>Weighted Pipeline</span><strong>{money(pipeline.weightedValue)}</strong></div><div><span>Won Revenue</span><strong>{money(pipeline.wonRevenue)}</strong></div></div></Card></div>
    <Card className="aim-upcoming"><div className="section-card-head"><span className="eyebrow">UPCOMING WORK</span><h2>A Follow-ups</h2><p>Next actions planned directly on the permanent lead record.</p></div>{data?.upcoming?.length ? <div className="lead-history-list">{data.upcoming.map((item) => <div className="lead-history-item" key={item.id}><div><Badge tone="info">{item.permanentLeadId}</Badge><strong>{item.companyName}</strong><span className="table-subtext">{item.nextAction}</span></div><span>{when(item.nextFollowUpDate)}</span></div>)}</div> : <EmptyState compact title="No upcoming A follow-ups" description="Add a next action and follow-up date to a master lead. C1/C2 activity follow-ups join this view in Phase 5." />}</Card>
  </div>;
}
